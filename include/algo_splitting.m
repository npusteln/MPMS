function [xn,crit] = algo_splitting_2017(param)

% linear operator
f1 = get_tv;
if strcmp(param.regtype,'nodiff')
    param.sigma = 1/(param.tau*(f1.beta+3));

elseif strcmp(param.regtype,'diff')
    param.sigma = 1/(param.tau*(f1.beta+3))/4;
...
while n<param.iter && (norm(xn(:)-xnold(:))^2/norm(xn(:)+eps)^2>param.epsilon || norm(xn(:)-xnold(:))/norm(xn(:)+eps)==0)
    ...
    % Update variables
    un    = f3.prox(xn - param.tau*(op_adj(yn1, param) + yn2 + yn3 +yn4), 1);
